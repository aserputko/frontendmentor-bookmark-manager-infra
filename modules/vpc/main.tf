...
# NAT Gateway (single shared in first public subnet)
...
resource "aws_route_table" "private" {
  count = var.enable_nat_gateway > 0 ? 1 : 0
...
resource "aws_route_table_association" "private" {
  count = var.enable_nat_gateway > 0 ? var.availability_zones : 0
...
resource "aws_cloudwatch_log_group" "vpc_flow_logs" {
  count = var.enable_vpc_flow_logs && var.flow_log_destination_type == "cloud-watch-logs" ? 1 : 0

  name              = "${var.flow_log_cloudwatch_log_group_name}/${var.environment}-${var.project_name}"
  retention_in_days = 7
